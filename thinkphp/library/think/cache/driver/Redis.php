 ...
 *
 *
 * 于20171110经过谭峰扩展，现在功能比较全了，涵盖redis的5种数据结构的操作，个体为如下改动：
 * 1、在__construct()方法中引入了redis的访问参数（IP,PORT,PASSWORD)；
 * 2、将__construct()方法私有化；
 * 3、增加静态实例化对象变量：private static $_instance = null; //单例对象
 * 4、增加获得单例实例化对象的静态方法：getInstance()
 *
 * 使用建议如下：
 * 第一步：建立一个控制器的父类(RedisBase.php)，本项目里已经建好
 * namespace tools;
 * use think\Controller;
 * use think\cache\driver\Redis;
 * class RedisBase extends Controller{
 ...
 * }
 * }
 *第二步：将要使用Redis的控制器继承这个RedisBase.php父类，如：
 * class IndexController extends RedisBase
 * 在该控制器中以下方式就可以访问了：
 * $this->redis->set('zai','肖哥');
 * $this->redis->handler()->lPush('namee','jack'); //原生的redis访问，函数全面，函数名一般与redis-cli中用的一致，见《书》上
 * $this->redis->has('zai')); //tp5扩展（即下述）的函数，虽经我在网上找了代码扩充，但仍然不是很全面
 */
class Redis extends Driver
{
    ...
    private function __construct($options = [])
    {
        ...
        //读redis配置文件，IP、port、password，这是我加上的*****
//        echo config('default_return_type');die(); //html
        $options = [
            'host'     => config('host'),
            'port'     => config('port'),
    ...
    {
        return $this->handler->delete($this->getCacheKey($name)); //phpstorm不够聪明，这时返回受影响的记录数，一般为1
    }
