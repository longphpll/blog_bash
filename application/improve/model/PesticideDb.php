 ...
 * 农药使用情况管理
 */
class PesticideDb extends BaseDb
{
    /*已改 Lxl*/
    static function add($data)
    {
        try {
            ...
            unset($data['id']);
            //插入记录并获得新增id
            $result = Db::table('improve_pesticide')->insertGetId($data);
    ...
    static function ls($data)
    {
        try {
            $where = 'm.status = 1';
            $field = 'm.id,m.region,m.region_name,m.unit,m.years,m.naturals,m.biochemistry,m.chemistry,m.germ,u.name adder,m.create_time';
            ...
            $dataRes = Db::table('improve_pesticide')->alias('m')
    ...
    {
        try {
            $dbRes = Db::table('improve_pesticide')->alias('m')
                ->join('frame_base_staff u', 'u.uid = m.adder', 'left')
                ->where('m.id', $id)
                ->where('m.status', 1)
                ->field('m.id,m.region,m.region_name,m.unit,m.years,m.naturals,m.biochemistry,m.chemistry,m.germ,u.name adder,m.create_time')
    ...
    static function edit($data)
    {
        try {
            ...
            $dbRes = Db::table('improve_pesticide')
                ->field('region,region_name,unit,naturals,biochemistry,chemistry,germ,years,adder,update_time')->update($data);
    ...
    {
        try {
            $dataRes = Db::table('improve_pesticide')->whereIn('id', $ids)->update(['status' => 2]);
    ...
    {
        try {
            $datasql = "(SELECT DISTINCT substring(region,1,9) region FROM b_pesticide where status = '1')";
    ...
    {
        try {
            $datasql = Db::table('improve_pesticide')
    ...
    //农药使用统计图
    static function PesticideList($data)
    {
        try {
            $dataRes = Db::table('improve_pesticide')
                ->field('SUM(naturals) naturals,SUM(biochemistry) biochemistrys,SUM(chemistry) chemistrys, SUM(germ) germs ')
                ->where('status', 1)
                ->whereLike('region', $data['region'] . '%')
                ->select();
    ...
    static function exportls($data, $field, $condition)
    {
        try {
            ...
            $dataRes = Db::table('improve_pesticide')->field($field)->where($where)->order($order)->select();
